...
(and thus able to run on you GPU). Moreover, OpenCAL allows for some
...
OpenCAL, discussing \emph{unsafe mode}'s operations.
...
Game of Life can be thought as an infinite two-dimensional orthogonal
grid of square cells, each of which is in one of two possible states,
\emph{dead} or \emph{alive}. Every cell interacts with the
eight\footnote{Note that, in the case of the Game of Life CA, the
  central cell does not belong to its own neighborhood.} adjecent
neighbors belonging to the Moore neighborhood. At each time step, one
...
create further generations. For more details on the Game of life you
can check Wikipedia at the URL
...
\begin{figure}
  \begin{center}
    ...
    \caption{OpenCAL's 2D cellular space and Moore neighborhood. Cells
      ...
      neighborhood is represented in gray scale, whit the central cell
      highlated in dark gray. Neighboring cells can also be indexed by
      the integer subscript shown within the cell. Cells indices are
      impliclitly assigned by OpenCAL, both in the case of predefined
      and custom neighborhoods. In the first case, indices can not be
      modified, while in the second case, indices as assigned
      progressively in automatic way each time a new neighbor is added
...
\item $X = \{(0,0), (-1, 0), (0, -1), (0, 1), (1, 0), (-1,-1), (1,-1),
  ...
  = (i-1, j)$, i.e. the first neighbour, and so on (see Figure
  \ref{fig:LifeNeighborhood}).
  
\item $Q = \{0, 1\}$ is the set of cell's states.
  
\item $\sigma : Q^8 \rightarrow Q$ is the deterministic cell
...
selected (see Figure \ref{fig:LifeNeighborhood}), among those provided
by OpenCAL. See Listings \ref{lst:CALNeighborhood2D} and
\ref{lst:CALNeighborhood3D} for a list of OpenCAL's 2D and 3D
neighborhoods, respectively. Custom neighborhoods can also be defined
by means of the \verb'calAddNeighbor2D()' function. In both cases,
indices are assigned progressively, starting from 0, each time a new
cell is added to the neighborhood..

The fourth parameter specifies the boundary
...
you to specify if your model has to use the so called \emph{active
  cells optimization}, that is able to restrict the computation to
only \emph{non-stationary cells}. In this case, no optimization is
considered.
