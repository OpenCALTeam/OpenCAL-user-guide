...
a more complex 2D example are illustrated, to show differet
...
\section{OpenCAL data types and statements convention}\label{sec:Conventions}
...
concurrently\footnote{On the serial version of OpenCAL, implicit
  ...
  planes built into OpenCAL substates. The first one, referred as
  \emph{current}, is used to read substates values for the central
  cell and its neighbors, while the second, that referred as
...
OpenCAL allows for an even further optimization of the SciddicaT
debris flows simulation model by means of the so called \emph{unsafe
...
formally only allows for state change for the central one. When such
kind of unsafe operations are adopted, we are in front of an XCA
eXtended Cellular Automata. Obviously, in order to be well defined,
the XCA must be equivalent to the original CA model.
...
Even if Listing \ref{lst:cal_mod2CA3D} is concise, it
