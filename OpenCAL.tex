...
has to use the so called \emph{active cells optimization}, that permit to restrict the computation to only \emph{non-stationary
...
performs substates' updates automatically, while in the second case which
substates' updates are executed can be chosen by the user. Note that, in case implicit
update policy is applied, all the CA substates are updated after the
...
detail. In particular, it takes the handle to the CA model to which the elementary
...
of a developer-defined function found at lines 14-24. As
...
a 3D CA) (cf. Listing \ref{lst:calAddNeighbor2D-3D}). By doing this, you
will start with an empty neighbouring pattern and can subsequently call the
...
receives the subscript identifier 0 and therefore can be used later
...
Eventually, note that you can define custom neighbourhoods starting from a predefined
one. For instance, let's still consider the above Game of Life
...
calling \verb'calAddNeighbor2D()' as many times the as the number of
...
simplified toy-version of Sciddica (SciddicaT in the following)
...
As noted, the \verb'calRunAddGlobalTransitionFunc2D()' function
...
As regards the computational performance, execution time of this further optimized
...
before. The obtained speed up can seem quite small (less than the 10\%)
with respect to the previous implementation of SciddicaT. However,
it must be noted that SciddicaT is a very simplified model and higher speed ups can certainly
be obtained for more complex CA made up by more elementary processes,
these latter involving here only a small set of model's substates. Table
...
In order to introduce you to three-dimensional CA development
with OpenCAL, this section describes the implementation of a simple 3D model,
namely the \emph{mod2} 3D CA. In this model, cells can be in one of two different
...
new state for the central cell as $s\%2$ (i.e. the remainder of $s$ divided
...
Lines 225 enters the graphic main loop thanks to the
...
As for the previous example in which we re-proposed the SciddicaT CA
application enriched by a 3D viever, here we re-propose the
...
\lstinputlisting[label=lst:calgl_mod2, caption=An OpenCAL
  ...
  output.]{../opencal/OpenCAL-GL/examples/calgl_mod2CA3D/source/mod2CA.c}
...
\verb'OpenCAL-GL/calgl3DWindow.h' headers are included and the
OpenCAL-GL functions, already seen in the 2D version into the previous
section, are present in the 3D form (see, e.g., the
\verb'calglAddToDrawModel3Db()' function at line 80). They are
completely equivalents to the 2D version.  Eventually, note that lines
86-91 are commented. If you remove the comment, you will cut down some
parts of the cellular space form visualization.


\section{OpenCAL Global Functions}\label{sec:redution}

OpenCAL comes with some functions you can use to perform global
operations over the cellular space. They essentially are reduction
...
callback functions. The proper use of global functions is your own
responsibility.
...
the OpenCAL's global functions. All of theese functions accepts a
pointer to a CA object as first paramenter, a pointer to a substate
over which execute the global operation as second parameter and return
the value corresponding to the executed reduction. For instance, if
you want to know the maximum value of the substate \verb'Q.h' of the
SciddicaT CA, you can write something like the code in Listing
\ref{lst:reduction_example}.
