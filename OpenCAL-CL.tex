...
functions, are defined device-side, due to their \textsl{parallel
...
not registered to the host-side CA model anymore. At the contrary, they are
registered to a \emph{device-side CA model}, that also embeds the
...
line 11). In the case the active cells optimization is considered, the
...
\begin{table}
  ...
  \begin{tabular}{l|l}
    ...
    Function & Brief description\\
    \hline
    \verb'calclAddInitFunc[2D|3D]'             & register a global kernel to be executed once before simulation loop \\
    \verb'calclAddElementaryProcess[2D|3D]()'  & register an cell elementary process \\
    \verb'calclAddSteeringFunc[2D|3D]'         & register a global kernel to be executed after each simulation step \\
...
Figures \ref{fig:calcl_life_0000} and \ref{fig:calcl_life_LAST} show the initial
