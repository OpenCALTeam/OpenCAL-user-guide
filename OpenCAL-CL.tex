...
1). According to OpenCL, each kernel definition must start with the
...
both a pointer to host and device-side CA as parameter and also a pointer
to an OpenCL kernel.
...
The \verb'calclRun{2D|3D}()' function (cf. Listings
...
is subsequently registered to the device-side CA by means of the
...
\ref{lst:calcl_reduction_example_device} show host- and device-side
...
\begin{lstlisting}[float, label=lst:calclAddReductionSum2Dr, caption=The calclAddReductionSum2Dr() host-side reduction registration function., numbers=none]
